...
import (
	...
	util "github.com/vinceflores/gitcallog/internal"
)
...
func InitModel(gitlog map[string][]string) Model {
	...
	for key, value := range gitlog {
		...
		d := time.Date(util.ParseInt(year), time.Month(util.ParseInt(month)), util.ParseInt(day), 0, 0, 0, 0, time.UTC)
